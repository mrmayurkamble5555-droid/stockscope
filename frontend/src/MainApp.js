      ...
      <div style={{ textAlign:"center" }}>
        <svg width="80" height="80" viewBox="0 0 80 80">
          ...
          <circle cx="40" cy="40" r="32" fill="none" stroke={color} strokeWidth="6"
            strokeDasharray={`${(1-rank/total)*201} 201`} strokeLinecap="round"
...
// ─── Market Mood Meter ─────────────────────────────────────────────────────────
// Uses Yahoo Finance ^NSEI (Nifty 50) — reliable, no auth needed
// Shows Bullish / Slightly Bullish / Neutral / Slightly Bearish / Bearish
function MarketMoodMeter() {
  const [mood,    setMood]    = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const YF_URL = "https://query1.finance.yahoo.com/v8/finance/chart/%5ENSEI?interval=1d&range=5d";
    const YF_HDR = { "User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36", "Accept":"application/json" };

    fetch(YF_URL, { headers: YF_HDR })
      .then(r => r.json())
      .then(data => {
        ...
        // +2% → score ~90 (bullish), 0% → 50 (neutral), -2% → ~10 (bearish)
        setMood({ score: Math.min(95, Math.max(5, 50 + chgPct * 20)), chg: chgPct.toFixed(2), price: currentPrice });
      })
      .catch(() => {
        // Backend OHLC fallback
        fetch(`${backendBase}/ohlc/NIFTY50`)
          ...
          .then(d => {
            ...
            if (candles.length >= 2) {
              ...
              setMood({ score: Math.min(95, Math.max(5, 50+chg*20)), chg: chg.toFixed(2), price: curr });
            } else {
              setMood({ score:55, chg:"0.00", price:null });
            }
          })
          .catch(() => setMood({ score:55, chg:"0.00", price:null }));
      })
      .finally(() => setLoading(false));
  }, []);

  const score = mood?.score ?? 55;
  const chg   = parseFloat(mood?.chg ?? "0");
  const price = mood?.price;

  // Clear sentiment language
  const getSentiment = (s) => {
    if (s >= 75) return { label:"Bullish",          emoji:"🐂", color:"#15803d", bg:"#dcfce7" };
    if (s >= 60) return { label:"Slightly Bullish", emoji:"📈", color:C.green,   bg:"#dcfce7" };
    if (s >= 42) return { label:"Neutral",          emoji:"➡️", color:C.amber,   bg:C.amberLight };
    if (s >= 28) return { label:"Slightly Bearish", emoji:"📉", color:"#f97316", bg:"#ffedd5" };
    return              { label:"Bearish",          emoji:"🐻", color:C.red,     bg:C.redLight };
  };
  const sent = getSentiment(score);
  ...
  return (
    <div style={{ ...S.card, padding:"16px 20px" }}>
      ...
      {loading ? (
        <div style={{ color:C.textThird, fontSize:12, textAlign:"center", padding:"20px 0" }}>Loading...</div>
      ) : (
        <>
          {/* Gauge SVG */}
          <svg width="140" height="80" viewBox="0 0 140 80" style={{ display:"block", margin:"0 auto" }}>
            ...
            <line x1="70" y1="70" x2={nx.toFixed(1)} y2={ny.toFixed(1)} stroke={C.textPrimary} strokeWidth="2.5" strokeLinecap="round" style={{ transition:"all .6s ease" }}/>
            <circle cx="70" cy="70" r="5" fill={C.textPrimary}/>
          </svg>

          {/* Sentiment badge */}
          <div style={{ textAlign:"center", marginTop:10 }}>
            <div style={{ display:"inline-flex", alignItems:"center", gap:6, background:sent.bg, borderRadius:20, padding:"6px 14px", marginBottom:8 }}>
              <span style={{ fontSize:16 }}>{sent.emoji}</span>
              <span style={{ fontSize:14, fontWeight:800, color:sent.color }}>{sent.label}</span>
            </div>
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, padding:"2px 8px", borderRadius:6, background:chg>=0?C.greenLight:C.redLight, color:chg>=0?C.greenText:C.redText }}>
                {chg>=0?"+":""}{chg}%
              </span>
            </div>

            {/* Zone labels */}
            <div style={{ display:"flex", justifyContent:"space-between", marginTop:10, fontSize:10, color:C.textThird, padding:"0 4px" }}>
              <span style={{ color:C.red,    fontWeight:600 }}>Bearish</span>
              <span style={{ color:C.amber,  fontWeight:600 }}>Neutral</span>
              <span style={{ color:"#15803d",fontWeight:600 }}>Bullish</span>
            </div>
          </div>
        </>
      )}
...
// ─── Trending Stocks ───────────────────────────────────────────────────────────
// Backend: GET /api/v1/trending?type=gainers|active  (backend/src/routes/trending.ts)
function TrendingStocks({ onSelectStock }) {
  ...
  return (
    <div style={S.card}>
      ...
      {error && !loading && <div style={{ color:C.textThird, fontSize:12, padding:"8px 0" }}>Could not load data. Check backend is running.</div>}
          ...
          {stocks.slice(0,8).map((s,i)=>(
            <div key={s.ticker||i} onClick={()=>onSelectStock(s.ticker)}
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                <div style={{ width:28, height:28, borderRadius:7, background:C.green, display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:800, color:"#fff" }}>
                  {(s.ticker||"?")[0]}
                </div>
                <div>
              ...
              <div style={{ textAlign:"right" }}>
                <div style={{ fontSize:13, fontWeight:700, color:C.textPrimary }}>{s.cmp?`₹${s.cmp}`:"-"}</div>
                {s.growth5Y!=null && (
...
function SearchBar({ onSearch, onSelectStock }) {
  ...
  const fetchSuggs = useCallback(async (q) => {
    ...
    try {
      ...
      setSuggs(await r.json() || []);
    } catch { setSuggs([]); }
    finally { setLoading(false); }
  }, []);

  const handleChange  = (e) => { const v=e.target.value.toUpperCase(); setQuery(v); setShowSugg(true); if(debounceRef.current) clearTimeout(debounceRef.current); debounceRef.current=setTimeout(()=>fetchSuggs(v),200); };
  const handleSelect  = (t) => { setQuery(t); setShowSugg(false); setSuggs([]); onSelectStock(t); };
  const handleSearch  = ()  => { setShowSugg(false); onSearch(query); };

  useEffect(()=>{ const h=(e)=>{ if(wrapRef.current&&!wrapRef.current.contains(e.target)) setShowSugg(false); }; document.addEventListener("mousedown",h); return()=>document.removeEventListener("mousedown",h); },[]);

  return (
    <div ref={wrapRef} style={{ position:"relative", flex:1 }}>
      <div style={{ display:"flex", gap:10 }}>
        <input value={query} onChange={handleChange} onFocus={()=>{ if(query.length>0) setShowSugg(true); }} onKeyDown={e=>e.key==="Enter"&&handleSearch()}
        ...
        <div style={{ position:"absolute", top:"calc(100% + 6px)", left:0, right:60, background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, boxShadow:"0 8px 24px rgba(0,0,0,0.12)", zIndex:100, overflow:"hidden" }}>
          ...
          {suggs.slice(0,8).map((s,i)=>(
            <div key={i} onMouseDown={()=>handleSelect(s.ticker)}
              style={{ padding:"10px 14px", cursor:"pointer", borderBottom:i<suggs.length-1?`1px solid ${C.border}`:"none", display:"flex", justifyContent:"space-between", alignItems:"center", transition:"background .1s" }}
              onMouseEnter={e=>e.currentTarget.style.background=C.greenLight}
...
// ─── Sector Screener ───────────────────────────────────────────────────────────
// Backend: GET /api/v1/screener                           → sector list
//          GET /api/v1/screener?sector=X&sort=Y&limit=200 → stocks
...
function Screener({ onSelectStock }) {
  ...
  useEffect(()=>{
    ...
    fetch(`${backendBase}/screener`)
      ...
      .then(d=>{ const s=(d.sectors||[]).filter(s=>s.sector&&s.count>0).sort((a,b)=>b.count-a.count); setSectors(s); })
  ...
  const loadSector = async (s, sort=sortBy, p=0) => {
    ...
    try {
      ...
      if(!r.ok) throw new Error(); const d=await r.json(); setStocks(d.stocks||[]); setPage(p);
    } catch { setStocks([]); } finally { setLoading(false); }
  };
  ...
  return (
    <div style={{ marginTop:8 }}>
      ...
      {loadingSectors && <div style={{ fontSize:13, color:C.textThird, padding:"16px 0" }}>Loading sectors...</div>}
      {sectorError&&!loadingSectors && <div style={{ fontSize:13, color:C.red, padding:"12px 0" }}>Could not load sectors. Make sure backend screener route is registered.</div>}
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12, fontFamily:"Inter,sans-serif" }}>
                  <thead>
                    <tr style={{ background:C.surfaceAlt }}>
                      {["#","Ticker","Company","CMP","P/E","D/E","Net Profit","1D Chg","52W H","52W L","Rank"].map(h=>(
                    ...
                    {pageStocks.map((s,i)=>{
                      ...
                      return (
                        <tr key={s.ticker} onClick={()=>onSelectStock(s.ticker)}
                          ...
                          <td style={{ padding:"9px 12px", color:s.change1D>=0?C.green:C.red, fontWeight:600 }}>{s.change1D!=null?`${s.change1D>=0?"+":""}${s.change1D?.toFixed(2)}%`:"-"}</td>
...
export default function App() {
  ...
  const loadStock = useCallback(async (ticker) => {
    ...
    try {
      ...
      setPriceRange(rR.ok ? await rR.json() : null);
      setActiveTab("overview"); setView("stock");
  ...
  const handleSearch = async (query) => {
    if(!query) return;
    try {
      const data = await (await fetch(`${backendBase}/search?q=${encodeURIComponent(query)}`)).json();
      setResults(data||[]);
              ...
              <section style={{ display:"grid", gridTemplateColumns:"minmax(0,1fr) 300px", gap:20 }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
                  ...
                  <div style={{ ...S.card, background:C.surface }}>
                    ...
                    <div style={{ marginTop:10, fontSize:10, color:C.textThird, lineHeight:1.5, borderTop:`1px solid ${C.border}`, paddingTop:8 }}>
